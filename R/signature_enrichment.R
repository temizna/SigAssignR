#' Calculate local context signature enrichment
#'
#' @description Calculate Mutation Signature Enrichment
#' @param inputdata a dataframe with columns of twentycontext (20 nucleotide context),
#'  con (trinucleotide context), sample (Sample names), 
#'  This dataframe can be created using either read_nonvcf function or
#'  vcfs_to_inputdata function
#'  @param context a list of 96 nucleotide contexts for signature enrichment i.e. "T[C>G]A"
#'  @param num_con Number of contexts to perform enrichment
#' @return enrichment table with adjusted p-values
#' @export
signature_enrichment<-function(inputdata, num_con, contexts){
