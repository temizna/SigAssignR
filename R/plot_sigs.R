...
#'
#' @export
plot_sigs<-function(fit, mode = c("relative", "absolute"), signatures=NA, logY=F){
  mode <- match.arg(mode)
  if(!(is.na(signatures))){
    fit=fit[,signatures]
  }
  ...
  if (mode == "absolute") {
    if (logY){
      bar_geom <- geom_bar(position=position_stack(reverse = TRUE),stat = "identity", 
                           colour = "black") 
      y_lab <- "Log( Absolute contribution \n (no. mutations))"
    } else {
  ...
  if (logY){
  plot <- ggplot(tb, aes(x = Sample, y = log10(Contribution+1), fill = Signature)) + 
